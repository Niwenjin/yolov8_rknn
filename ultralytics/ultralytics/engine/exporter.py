...
class Exporter:
    ...
    def get_int8_calibration_dataloader(self, prefix=""):
        ...
        dataset = YOLODataset(
            ...
            * 2,  # NOTE TensorRT INT8 calibration should use 2x batch size
        )
        n = len(dataset)
        if n < 300:
            LOGGER.warning(
        ...
        return build_dataloader(
            dataset, batch=self.args.batch * 2, workers=0
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Export a YOLOv model to a specific format."
    )
    parser.add_argument(
        "model_path", type=str, help="Path to the model file (e.g., mymodel.pt)"
    )
    # parser.add_argument(
    #     "--format",
    #     type=str,
...
def export(model_path, cfg=DEFAULT_CFG):
    """Export a YOLOv model to a specific format."""
    model_path = model_path or cfg.model
    cfg.format = cfg.format or "torchscript"
    from ultralytics import YOLO
